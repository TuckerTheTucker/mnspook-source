...
class Stage extends FlxTypedGroup<FlxBasic>
{
	...
	public var space:FNFSprite;
	public var back:FNFSprite;
	public var front:FNFSprite;
	public var bgcharmid:FNFSprite;
	public var bgcharfront:FNFSprite;
	...
	public function new(curStage)
	{
		...
		switch (curStage)
		{
			...
			case 'streetSpooky':
				...
				var texPath = 'backgrounds/$curStage/';
				space = new FNFSprite(-420, -320);
				space.frames = Paths.getSparrowAtlas(texPath + 'space');
				space.animation.addByPrefix('idle', 'space0', 24, false);
				space.animation.addByPrefix('lightning', 'space-lightning', 12, false);
				space.scrollFactor.set();
				add(space);

				back = new FNFSprite(-500, -270);
				back.frames = Paths.getSparrowAtlas(texPath + 'back');
				back.animation.addByPrefix('idle', 'back0', 24, false);
				back.animation.addByPrefix('lightning', 'back-lightning', 12, false);
				back.scrollFactor.set(0.5, 1);
				add(back);

				front = new FNFSprite(-710, -230);
				front.frames = Paths.getSparrowAtlas(texPath + 'front');
				front.animation.addByPrefix('idle', 'front0', 24, false);
				front.animation.addByPrefix('lightning', 'front-lightning', 12, false);
				front.scrollFactor.set(1, 1);
				add(front);

				bgcharmid = new FNFSprite(-360, 200);
				bgcharmid.frames = Paths.getSparrowAtlas(texPath + 'bgchar');
				bgcharmid.animation.addByPrefix('idle', 'mid-bg', 24, false);
				bgcharmid.scrollFactor.set(1, 1);
				add(bgcharmid);
				bgcharmid.playAnim('idle', true);

				bgcharfront = new FNFSprite(-1070, 340);
				bgcharfront.frames = bgcharmid.frames;
				bgcharfront.animation.addByPrefix('idle', 'front-bg', 24, false);
				bgcharfront.scrollFactor.set(1.2, 1);
				foreground.add(bgcharfront);
				bgcharfront.playAnim('idle', true);
			default:
	...
	public function stageUpdate(curBeat:Int, boyfriend:Boyfriend, gf:Character, dadOpponent:Character)
	{
		...
		switch (PlayState.curStage)
		{
			...
			case 'streetSpooky':
			{
				bgcharmid.playAnim('idle', true);
				bgcharfront.playAnim('idle', true);
				//yoshubs why must you delete the lightning code now i gotta add it back and modify it grrrr
				if (FlxG.random.bool(10) && curBeat > lightningStrikeBeat + lightningOffset)
				{
					lightningStrikeShit(boyfriend, gf, curBeat);
				}
			}
		}
	}
	...
	function lightningStrikeShit(bf:Boyfriend, gf:Character, beat:Int):Void
	{
		FlxG.sound.play(Paths.soundRandom('thunder_', 1, 2));

		space.animation.play('lightning', true);
		back.animation.play('lightning', true);
		front.animation.play('lightning', true);
