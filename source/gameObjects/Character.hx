...
class Character extends FNFSprite
{
	...
	public function setCharacter(x:Float, y:Float, character:String):Character
	{
		...
		switch (curCharacter)
		{
			...
			case 'wicherus':
					frames = Paths.getSparrowAtlas('characters/wicherus');
					animation.addByPrefix('idle', 'witch idle', 24, false);
					animation.addByPrefix('singUP', 'witch up', 24, false);
					animation.addByPrefix('singDOWN', 'witch down', 24, false);
					animation.addByPrefix('singLEFT', 'witch left', 24, false);
					animation.addByPrefix('singRIGHT', 'witch right', 24, false);
		...
		//this code is the reason why you gotta have this along with newchartingstate, i wish i knew how to put this in the charting state
		switch (curCharacter)
		{
		...
		switch (curCharSimplified)
		{
			case 'gf':
				...
				if ((animation.curAnim.name.startsWith('sad')) && (animation.curAnim.finished))
					playAnim('danceLeft');
				if ((animation.curAnim.name == 'scared') && (animation.curAnim.finished))
					playAnim('danceLeft');
			case 'bf':
				if ((animation.curAnim.name == 'scared') && (animation.curAnim.finished))
					playAnim('idle');
