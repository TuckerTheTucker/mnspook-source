...
class OptionsMenuState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		categoryMap = [
			...
			'appearance' => [
				[
					...
					['', null],
					['Toggle Shader Filter', getFromOption],
		...
		infoText.setFormat(Paths.font('splatter.otf'), 20, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if (Init.gameSettings.get(activeSubgroup.members[curSelection].text) != null)
		{
			switch (Init.gameSettings.get(activeSubgroup.members[curSelection].text)[1])
			{
				...
				case Init.SettingTypes.Selector:
					...
					if (activeSubgroup.members[curSelection].text == 'Hitsound')
	...
	function updateSelector(selector:Selector, updateBy:Int)
	{
		...
		{ 
			...
			soundPath = ForeverTools.nabSoundFromString(Init.trueSettings.get('Hitsound'));
			if (activeSubgroup.members[curSelection].text == 'Hitsound')
				if(Init.trueSettings.get('Hitsound') != 'none')
					FlxG.sound.play(Paths.sound('hitsounds/hit-$soundPath'));
			Init.saveSettings();
		}
	}
