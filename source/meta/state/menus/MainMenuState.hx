...
class MainMenuState extends MusicBeatState
{
	...
	var vignetteBG:FlxSprite;
	var menuItem:FlxSprite;
	var camFollow:FlxObject;

	var tweenVar:FlxTween = null;
	...
	override function create()
	{
		...
		{
			menuItem = new FlxSprite(0, 80 + (i * 200));
			menuItem.frames = tex;
			// add the animations in a cool way (real
			menuItem.animation.addByPrefix('idle', optionShit[i] + " basic", 24);
			menuItem.animation.addByPrefix('selected', optionShit[i] + " white", 24);
			menuItem.animation.play('idle');
			menuItem.angle = (menuItem.ID % 2 == 0 ? -5 : 5);
			canSnap[i] = -1;
			// set the id
			menuItem.ID = i;
			menuItem.alpha = 0;
			...
			FlxTween.tween(menuItem, {alpha: 1, x: ((FlxG.width / 2) - (menuItem.width / 2))}, 1, {
		...
		vignetteBG = ForeverAssets.addVignette();
		add(vignetteBG);

		// from the base game lol
		var versionShit:FlxText = new FlxText(5, FlxG.height - 18, 0, "Spookin v" + Main.gameVersion, 12);
		versionShit.scrollFactor.set();
		versionShit.setFormat(Paths.font("splatter.otf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);
		//
	}
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.FIVE)
			Main.switchState(this, new OffsetState('bf', true));
		#end
	...
	private function updateSelection()
	{
		...
		tweenVar = FlxTween.tween(menuItems.members[Math.floor(curSelected)], {"scale.x": 1.1, "scale.y": 1.1}, 1, {type: PINGPONG, ease: FlxEase.quadInOut});
