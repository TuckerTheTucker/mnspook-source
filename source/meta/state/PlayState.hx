...
class PlayState extends MusicBeatState
{
	...
	public static var vocalsBF:FlxSound;
	public static var jumpSound:FlxSound;
	public static var healSound:FlxSound;
	...
	private var hudBeat:Int = 1;
	private var camBeat:Int = 4;
	...
	public static var cameraSpeed:Float = 1.5;
	...
	override public function create()
	{
		...
		//hitsound stuff
		hitPath = ForeverTools.nabSoundFromString(Init.trueSettings.get('Hitsound'));
		...
		var blue:FlxSprite = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, 0xFF004B82);
		blue.scrollFactor.set();
		blue.cameras = [camHUD];
		blue.alpha = 0.2;
		add(blue);

		//we dont use ForeverAssets.addVignette to get the vignette since we wanna tie it to the hud
		var vignetteBG:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menus/base/vignetteBG'));
		vignetteBG.scrollFactor.set();
		vignetteBG.setGraphicSize(Std.int(vignetteBG.width * 1.1));
		vignetteBG.updateHitbox();
		vignetteBG.screenCenter();
		vignetteBG.cameras = [camHUD];
		vignetteBG.antialiasing = true;
		add(vignetteBG);
		...
		if (Init.trueSettings.get('Toggle Shader Filter')) {
			var shader:GraphicsShader = new GraphicsShader("", File.getContent("./assets/shaders/spookybloom.frag"));
			FlxG.camera.setFilters([new ShaderFilter(shader)]);
		}

		jumpscareImage = new FlxSprite().loadGraphic(Paths.image("UI/default/base/jumpscare"));
		jumpscareImage.setGraphicSize(Std.int(jumpscareImage.width * 1.4));
		jumpscareImage.screenCenter();
		jumpscareImage.scrollFactor.set();
	...
	{
		//this is all the creative mode code is lmao
		if (FlxG.save.data.creativeMode) {
			camFollow.setPosition(boyfriend.getMidpoint().x - 100, boyfriend.getMidpoint().y - 100);

			var mm:Int = (FlxG.keys.pressed.SHIFT ? 5 : 1);
		...
		if (!inCutscene) {
			...
			if (!isStoryMode)
			{
				...
				if ((FlxG.keys.justPressed.SEVEN) && (!startingSong))
				{
					...
					if (FlxG.keys.pressed.SHIFT)
						...
						Main.switchState(this, new NewChartingState());
				}
			...
			if ((health <= 0 && startedCountdown) || (controls.RESET && startedCountdown))
			{
	...
	function noteCalls()
	{
		...
		if (generatedMusic && startedCountdown)
		{
			for (strumline in strumLines)
			{
				...
				{
					// set the notes x and y, modified for what i wanna dub "Bothscroll" seen in Bloodbrothers
					if (daNote.mustPress) {
					...
					if (daNote.isSustainNote) {
						...
						if ((daNote.animation.curAnim.name.endsWith('holdend')) && (daNote.prevNote != null)) {
							...
							if (downscrollMultiplier == -1) { //i hope this works
					...
					if ((((downscrollMultiplier == 1) && (daNote.y < -daNote.height))
					|| ((downscrollMultiplier == -1) && (daNote.y > (FlxG.height + daNote.height))))
	...
	{
		//if autoplay is on and a trick note is there, dont delete the note
		if (boyfriendStrums.autoplay && daNote.noteType == 1)
			return;
	...
	function goodNoteHit(coolNote:Note, character:Character, characterStrums:Strumline, ?canDisplayJudgement:Bool = true)
	{
		if (!coolNote.wasGoodHit) {
			...
			if (!coolNote.isSustainNote)
				if(Init.trueSettings.get('Hitsound') != 'none')
					FlxG.sound.play(Paths.sound('hitsounds/hit-$hitPath'));
			...
			//putting opponent's splashes here because splashes only appear when the score pops up (which only appears for bf)
			if (characterStrums == dadStrums) {
				if (!coolNote.isSustainNote) {
					createSplash(coolNote, characterStrums);
					//its not a spooky mod without health drain -some guy
					health -= 0.01;
				}
			}
	...
	function characterPlayAnimation(coolNote:Note, character:Character)
	{
		...
		//fuck it, no more alt notes

		character.playAnim(baseString, true);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		jumpSound = FlxG.sound.load(Paths.sound('JUMPSCARE'));
		jumpSound.persist = true;
		healSound = FlxG.sound.load(Paths.sound('heal'));
		healSound.persist = true;
		if (SONG.needsVoicesDad)
			vocalsDad = new FlxSound().loadEmbedded(Paths.voicesDad(SONG.song), false, true);
		else
			vocalsDad = new FlxSound();

		if (SONG.needsVoicesBF)
			vocalsBF = new FlxSound().loadEmbedded(Paths.voicesBF(SONG.song), false, true);
		else
	...
	override function beatHit()
	{
		...
		if ((FlxG.camera.zoom < 1.35 && curBeat % camBeat == 0) && (!Init.trueSettings.get('Reduced Movements')))
		{
	...
	function endSong():Void
	{
		...
		if (curSong.toLowerCase() == 'treat') //publishing day is tonight i dont have time to clean up code
			songEndSpecificActions();

		if (!isStoryMode)
		{
			if (curSong.toLowerCase() != 'treat')
	...
	private function callDefaultSongEnd()
	{
		...
		ForeverTools.killMusic([songMusic, vocalsDad, vocalsBF]);
	...
	//i made end dialogue stuff, ONLY USE IN THE END OF A SONG!
	function callTextbox(?end:Bool = false) {
		if (skipCutscenes())
			return returnToMenu();

		var dialogPath = Paths.json(SONG.song.toLowerCase() + (end ? '/dialogueEND' : '/dialogue'));
		if (sys.FileSystem.exists(dialogPath))
		{
			...
			if (!end)
				...
				dialogueBox.whenDaFinish = returnToMenu;
	...
	var isTricked:Bool = false;

	function trick():Void
	{
		trace('TRICKED!!');
		isTricked = true;
		FlxG.camera.shake(0.01, Conductor.crochet / 1000);
		for (hud in allUIs)
			hud.visible = false;
	...
	{
		trace('TREATED!!');
		FlxG.camera.fade(0x80FFFFFF, 0.6, true);
		healSound.play(true);
		health += 1;
	}
